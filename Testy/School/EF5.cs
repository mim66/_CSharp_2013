    ...
    class EF5
    {
        ...
        public static void ReadDataUsingStoredProcedure()
        {
					...
					using (var context = new SchoolDBEntities())
					{
						...
						var courses = context.GetCourseByStudenttId(1);
				...
				//public static void ReadDataUsingStoredProcedure2()
				//{
				//	Console.WriteLine("*** ReadDataUsingStoredProcedure Start ***");
				//	using (var context = new SchoolDBEntities())
				//	{
				//		var courses = context.GetCoursesByStudentId_Course(1);
				//		Console.WriteLine("		Kursy:");
        ...
        public static void LazyLoadingDemo()
        {
            Console.WriteLine("*** LazyLoadingDemo Start ***");
						...
						using (var context = new SchoolDBEntities())
						{
							...
							StudentAddress adr = std.StudentAddress;
							Console.WriteLine("		Liczba studentów: {0}; Student: {1}; Adresy: {2}", studList.Count, std.StudentName, adr == null ? "null" : adr.Address1);

							//Standard std = student.Standard;
							//Console.WriteLine("			Liczba studentów: {0}; Student: {1}; Standard: {2}", studList.Count, student.StudentName, std == null ? "null": std.StandardName);
						}
        ...
        public static void ExplicitLoadingDemo()
        {
            ...
            using (var context = new SchoolDBEntities())
            {
                ...
                Student std = studList.Where(s => s.StudentID == 1).FirstOrDefault<Student>();
								Console.WriteLine("		Liczba studentów: {0}; Student: {1}", studList.Count, std.StudentName);

                //Loads Standard for particular Student only (seperate SQL query)
                context.Entry(std).Reference(s => s.Standard).Load();
								Console.WriteLine("		StandardName: {0};", context.Standards.FirstOrDefault<Standard>().StandardName);

                //Loads Courses for particular Student only (seperate SQL query)
                context.Entry(std).Collection(s => s.Courses).Load();
								Console.WriteLine("		CourseName: {0};", context.Courses.FirstOrDefault<Course>().CourseName);
						}
        ...
        public static void RawSQLtoEntityTypeDemo()
        {
            ...
            using (var context = new SchoolDBEntities())
            {
                ...
                var studentName = context.Students.SqlQuery("Select StudentId, StudentName, StandardId, RowVersion from Student where StudentId =1").ToList();
								Console.WriteLine("		Liczba studentów: {0}; Student: {1}", studentList.Count, studentName.FirstOrDefault<Student>().StudentName);
            }
        ...
        public static void EntitySQLDemo()
        {
            ...
            using (var context = new SchoolDBEntities())
            {
                string sqlString = "SELECT VALUE st  FROM SchoolDBEntities.Students AS st  WHERE st.StudentID = 1";
                var objctx = (context as IObjectContextAdapter).ObjectContext;

                ObjectQuery<Student> student = objctx.CreateQuery<Student>(sqlString);
                Student std = student.FirstOrDefault<Student>();
								Console.WriteLine("		StudentId: {0}; StudentName: {1}", std.StudentID, std.StudentName);
        ...
        public static void SpatialDataTypeDemo()
        {
            ...
            using (var ctx = new SchoolDBEntities())
            {
                ...
                ctx.SaveChanges();
								var course = ctx.Courses.Where(c => c.CourseName == "New Course from SpatialDataTypeDemo").Single();
								Console.WriteLine("Course Name: {0}, Course Location: {1}", course.CourseName, course.Location);
